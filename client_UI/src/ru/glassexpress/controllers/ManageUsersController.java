...
public class ManageUsersController extends BaseController {
    ...
    public void delUser(ActionEvent actionEvent) {
        ...
        boolean isTrue = AlertWindow.confirmationWindow("Вы уверены?", "Удалить " + emps.get(getSelectedIndex()) + " из базы?");
        if (isTrue) {
            isTrue = AlertWindow.confirmationWindow("Вы точно уверены?", "Будут стерты все записи о сотруднике!");
            if (isTrue) {
